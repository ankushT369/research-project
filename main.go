...
// ==================== Chaos-Based Logistic Map Cipher ====================

type LogisticChaosCipher struct {
	keySize int // seed size (8 bytes for float64)
}

func NewLogisticChaosCipher() *LogisticChaosCipher {
	return &LogisticChaosCipher{
		keySize: 8, // 8 bytes seed
	}
}

func (c *LogisticChaosCipher) Name() string { return "CHAOS-LOGISTIC" }

// Generate chaotic keystream using logistic map
func (c *LogisticChaosCipher) generateKeystream(seed []byte, length int) []byte {
	// Convert seed to initial x0 value (between 0.1 and 0.9)
	seedVal := float64(0)
	for i := 0; i < len(seed) && i < 8; i++ {
		seedVal = seedVal*256 + float64(seed[i])
	}
	// Normalize to [0.1, 0.9]
	x0 := 0.1 + (math.Mod(seedVal, 0.8))
	
	// r value in chaotic regime [3.57, 4.0]
	r := 3.57 + (math.Mod(seedVal*1.7, 0.43))
	
	// Iterate logistic map to generate keystream
	x := x0
	keystream := make([]byte, length)
	
	// Skip first 100 iterations to avoid transient behavior
	for i := 0; i < 100; i++ {
		x = r * x * (1 - x)
	}
	...
	for i := 0; i < length; i++ {
		...
		keystream[i] = byte(val)
	}
	
	// for i := 0; i < length; i++ {
	// 	x = r * x * (1 - x)
	// 	// Extract 8 bits from the chaotic value
	// 	keystream[i] = byte(math.Floor(x*256)) % 256
	// }
	
	return keystream
}

func (c *LogisticChaosCipher) Encrypt(plaintext []byte) ([]byte, []byte, error) {
	// Generate random seed
	seed := make([]byte, c.keySize)
	if _, err := io.ReadFull(rand.Reader, seed); err != nil {
		return nil, nil, err
	}
	
	// Generate keystream
	keystream := c.generateKeystream(seed, len(plaintext))
...
func (c *LogisticChaosCipher) Decrypt(key []byte, ciphertext []byte) ([]byte, error) {
	// Generate same keystream from seed
	keystream := c.generateKeystream(key, len(ciphertext))
...
func (cl *CryptoLayer) RecoverMessageFromSecret(secretStr string) (string, error) {
	...
	// Skip cipher name check for now, just try to decrypt
	cipherName := parts[0]
	log.Printf("Decrypting with cipher: %s", cipherName)
...
func getCipher(name string) Cipher {
	...
	case "CHAOS", "CHAOS-LOGISTIC":
		return NewLogisticChaosCipher()
	default:
...
func (s *Server) handleGenerate(c *gin.Context) {
	...
	cipherName := c.PostForm("cipher")

	k, _ := strconv.Atoi(kStr)
	n, _ := strconv.Atoi(nStr)
	m, _ := strconv.Atoi(mStr)

	log.Printf("Generating: k=%d, n=%d, m=%d, cipher=%s", k, n, m, cipherName)
...
func (s *Server) handleReconstruct(c *gin.Context) {
	...
	log.Printf("Reconstructing: k=%d, m=%d, cipher=%s", k, m, cipherName)
	...
	// Get the same cipher used for encryption
	cipher := getCipher(cipherName)
	cryptoLayer := NewCryptoLayer(cipher)
	message, err := cryptoLayer.RecoverMessageFromSecret(reconstructedSecret)
	if err != nil {
...
func main() {
	...
	log.Println("Secret Sharing Scheme - ChaCha20 & Chaos-Logistic Ciphers")
